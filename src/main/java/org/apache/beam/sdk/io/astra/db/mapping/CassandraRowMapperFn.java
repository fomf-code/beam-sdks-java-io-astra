...
class CassandraRowMapperFn implements Mapper<Row>, Serializable {
     ...
     */
    CassandraRowMapperFn(Session session, ValueProvider<String> keyspace, ValueProvider<String> table) {
    ...
    public Iterator<Row> map(ResultSet resultSet) {
        ...
        if (schema == null) {
            // Get the order of the primary keys.
            // We store the order of the primary keys in the field metadata.
            Map<String, Integer> keyOrder = CassandraKeyUtils.primaryKeyOrder(session, keyspace.get(), table.get());
    ...
    public Future<Void> deleteAsync(Row entity) {
        /*
        StringBuilder deleteQuery = new StringBuilder(String.format("DELETE FROM %s.%s WHERE ", keyspace.get(), table.get()));
        Map<String, Integer> keys = CassandraKeyUtils.primaryKeyOrder (session, keyspace.get(), table.get());
        for (Entry<String, Integer> entry : keys.entrySet()) {
            String cqlFieldValue;
            Schema.FieldType type = entity.getSchema().getField(entry.getKey()).getType();
            if (FieldType.DATETIME.equals(type)) {
                cqlFieldValue = String.valueOf(((DateTime) entity.getValue(entry.getKey())).getMillis());
            } else if (FieldType.STRING.equals(type)) {
                cqlFieldValue = "'" + entity.getValue(entry.getKey()) + "'";
            } else if (FieldType.BOOLEAN.equals(type) ||
                    FieldType.BYTES.equals(type) ||
                    FieldType.DECIMAL.equals(type) ||
                    FieldType.DOUBLE.equals(type) ||
                    FieldType.FLOAT.equals(type) ||
                    FieldType.INT16.equals(type) ||
                    FieldType.INT32.equals(type)) {
                cqlFieldValue = String.valueOf(entity.getValue(entry.getKey()));
            } else {
                throw new UnsupportedOperationException("Datatype " + type.getTypeName().name() + " not supported.");
            }
            deleteQuery.append(String.format("'%s' = %s AND ", entry.getKey(), cqlFieldValue));
        }

        return FutureUtils
                .asCompletableFuture(session.executeAsync(deleteQuery.toString())).thenApply(resultSet -> null);
                */
